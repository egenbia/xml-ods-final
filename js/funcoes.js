...
function corpoPostagem(){
    ...
    i = parametro.get("texto"); 

   var corpo = y[i].getElementsByTagName("texo")[0].childNodes[0].nodeValue;
...
function tituloPostagem(){
	...
	document.write(y[i].getElementsByTagName("textoODS")[0].childNodes[0].nodeValue)
}

function dataPostagem(){
    url = new URL(window.location.href)
    parametro = url.searchParams;
    i = parametro.get("metas"); 
...
function imagemPostagem(){
    ...
    document.write("<center><img src='imgs/"+ y[i].getElementsByTagName("logo")[0].childNodes[0].nodeValue +"' width='80%' style='margin-top: 00px;border-radius: 10px;'></center>")
}
...
function fontePostagem(){
	...
	document.write("fonte: "+y[i].getElementsByTagName("3.1")[0].childNodes[0].nodeValue)
}